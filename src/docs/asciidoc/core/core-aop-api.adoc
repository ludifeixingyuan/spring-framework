...
----
	...
	<bean id="person"
		...
		<property name="proxyInterfaces" value="com.mycompany.com.wz.test.Person"/>
...
The `person` bean definition shown earlier can be used in place of a `com.wz.test.Person` implementation, as
follows:

[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	com.wz.test.Person person = (com.wz.test.Person) factory.getBean("person");
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	val person = factory.getBean("person") as com.wz.test.Person;
----
...
The `PersonUser` class in this example exposes a property of type `com.wz.test.Person`. As far as
...
Using an anonymous inner bean has the advantage that there is only one object of type `com.wz.test.Person`. This is useful if we want
...
Imagine that in our earlier example, there was no `com.wz.test.Person` interface. We needed to advise
a class called `com.wz.test.Person` that did not implement any business interface. In this case, you
