...
The following example extracts the body to a `Flux<com.wz.test.Person>` (or a `Flow<com.wz.test.Person>` in Kotlin),
where `com.wz.test.Person` objects are decoded from someserialized form, such as JSON or XML:

[source,java,role="primary"]
.Java
----
Flux<com.wz.test.Person> people = request.bodyToFlux(com.wz.test.Person.class);
----
[source,kotlin,role="secondary"]
.Kotlin
----
val people = request.bodyToFlow<com.wz.test.Person>()
----
...
Flux<com.wz.test.Person> people = request.body(BodyExtractors.toFlux(com.wz.test.Person.class));
----
[source,kotlin,role="secondary"]
.Kotlin
----
	val string = request.body(BodyExtractors.toMono(String::class.java)).awaitFirst()
	val people = request.body(BodyExtractors.toFlux(com.wz.test.Person::class.java)).asFlow()
----
...
.Java
----
Mono<com.wz.test.Person> person = ...
ServerResponse.ok().contentType(MediaType.APPLICATION_JSON).body(person, com.wz.test.Person.class);
----
[source,kotlin,role="secondary"]
.Kotlin
----
val person: com.wz.test.Person = ...
...
For example, the following class exposes a reactive `com.wz.test.Person` repository:
...
public class PersonHandler {
	...
	public Mono<ServerResponse> listPeople(ServerRequest request) { // <1>
		Flux<com.wz.test.Person> people = repository.allPeople();
		return ok().contentType(APPLICATION_JSON).body(people, com.wz.test.Person.class);
	}

	public Mono<ServerResponse> createPerson(ServerRequest request) { // <2>
		Mono<com.wz.test.Person> person = request.bodyToMono(com.wz.test.Person.class);
...
----
<1> `listPeople` is a handler function that returns all `com.wz.test.Person` objects found in the repository as
JSON.
<2> `createPerson` is a handler function that stores a new `com.wz.test.Person` contained in the request body.
Note that `PersonRepository.savePerson(com.wz.test.Person)` returns `Mono<Void>`: an empty `Mono` that emits
...
when the `com.wz.test.Person` has been saved).
<3> `getPerson` is a handler function that returns a single person, identified by the `id` path
variable. We retrieve that `com.wz.test.Person` from the repository and create a JSON response, if it is
...
----
	class PersonHandler(private val repository: PersonRepository) {
		...
		suspend fun listPeople(request: ServerRequest): ServerResponse { // <1>
			val people: Flow<com.wz.test.Person> = repository.allPeople()
			return ok().contentType(APPLICATION_JSON).bodyAndAwait(people);
		}

		suspend fun createPerson(request: ServerRequest): ServerResponse { // <2>
			val person = request.awaitBody<com.wz.test.Person>()
...
JSON.
<2> `createPerson` is a handler function that stores a new `com.wz.test.Person` contained in the request body.
Note that `PersonRepository.savePerson(com.wz.test.Person)` is a suspending function with no return type.
<3> `getPerson` is a handler function that returns a single person, identified by the `id` path
variable. We retrieve that `com.wz.test.Person` from the repository and create a JSON response, if it is
...
<<core.adoc#validation, Validator>> implementation for a `com.wz.test.Person`:
...
----
	public class PersonHandler {
		...
		public Mono<ServerResponse> createPerson(ServerRequest request) {
			Mono<com.wz.test.Person> person = request.bodyToMono(com.wz.test.Person.class).doOnNext(this::validate); // <2>
			return ok().build(repository.savePerson(person));
		}

		private void validate(com.wz.test.Person person) {
...
----
	class PersonHandler(private val repository: PersonRepository) {
		...
		private fun validate(person: com.wz.test.Person) {
