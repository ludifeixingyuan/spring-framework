...
The Spring MVC com.wz.test.Test framework builds on the mock Servlet API objects to provide an
...
<<spring-mvc-test-framework, Spring MVC com.wz.test.Test Framework>> instead.
...
com.wz.test.Test classes typically declare either an array of resource locations for XML or Groovy
...
[[testing-fixture-di]]
==== Dependency Injection of com.wz.test.Test Fixtures
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@com.wz.test.Test(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles test execution timeouts
(that is, by executing the test method in a separate `Thread`), `@com.wz.test.Test(timeout=...)`
...
Since JUnit Jupiter supports the use of `@com.wz.test.Test`, `@RepeatedTest`, `ParameterizedTest`,
...
the `@com.wz.test.Test` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@com.wz.test.Test // org.junit.jupiter.api.com.wz.test.Test
...
* com.wz.test.Test instance post-processing.
...
==== com.wz.test.Test Execution Events
...
for which it is responsible. com.wz.test.Test instances do not automatically receive access to the
...
com.wz.test.Test classes that use the TestContext framework do not need to extend any particular
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with com.wz.test.Test Property Sources
...
====== Declaring com.wz.test.Test Property Sources
...
com.wz.test.Test properties have higher precedence than those defined in the operating system's
...
====== Inheriting and Overriding com.wz.test.Test Property Sources
...
.com.wz.test.Test suites and forked processes
[NOTE]
====
...
===== com.wz.test.Test-managed Transactions

com.wz.test.Test-managed transactions are transactions that are managed declaratively by using the
...
* JUnit 4's `@com.wz.test.Test(timeout = ...)` support and `TimeOut` rule
* JUnit Jupiter's `assertTimeoutPreemptively(...)` methods in the
  `org.junit.jupiter.api.Assertions` class
* TestNG's `@com.wz.test.Test(timeOut = ...)` support
====
...
hierarchy runs within a transaction. com.wz.test.Test methods that are not annotated with
...
----
	...
	@Transactional
	@com.wz.test.Test // no expected exception!
...
==== Parallel com.wz.test.Test Execution
...
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@com.wz.test.Test`, `@RepeatedTest`,
...
[[spring-mvc-test-framework]]
=== Spring MVC com.wz.test.Test Framework

The Spring MVC com.wz.test.Test framework provides first class support for testing Spring MVC code
...
Spring MVC com.wz.test.Test also provides client-side support for testing code that uses the
...
The goal of Spring MVC com.wz.test.Test is to provide an effective way to test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC com.wz.test.Test builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
...
Jupiter-based example uses Spring MVC com.wz.test.Test:
...
In Spring MVC com.wz.test.Test, async requests can be tested by asserting the produced async value
...
There are no options built into Spring MVC com.wz.test.Test for container-less testing of streaming
...
===== Spring MVC com.wz.test.Test vs End-to-End Tests

Spring MVC com.wz.test.Test is built on Servlet API mock implementations from the
...
There are pros and cons for each approach. The options provided in Spring MVC com.wz.test.Test are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC com.wz.test.Test fall under the category of classic unit
...
Another important distinction when using Spring MVC com.wz.test.Test is that, conceptually, such
...
sample tests] intended to show how to use Spring MVC com.wz.test.Test. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full test coverage based on Spring MVC com.wz.test.Test.
...
* com.wz.test.Test JavaScript within pages.
...
With Spring MVC com.wz.test.Test, we can easily test if we are able to create a `Message`, as follows:
...
To use Selenium WebDriver with the Spring MVC com.wz.test.Test framework, make sure that your project
...
----
	...
	// com.wz.test.Test code that uses the above RestTemplate ...
...
Spring MVC com.wz.test.Test's own tests include
...
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/com.wz.test.Test%20Spy.html[com.wz.test.Test Spy] pattern. Used by the Spring Framework
  in its test suite.
