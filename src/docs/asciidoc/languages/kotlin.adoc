...
----
    ...
    class PersonRepository(private val operator: TransactionalOperator) {
        ...
        private fun findPeople(): Flow<com.wz.test.Person> {
            // SELECT SQL statement
        }

        private suspend fun updatePerson(person: com.wz.test.Person): com.wz.test.Person {
...
As the following example shows, this allows for easy changes to individual properties, even if `com.wz.test.Person` properties are read-only:

[source,kotlin,indent=0]
----
	data class com.wz.test.Person(val name: String, val age: Int)

	val jack = com.wz.test.Person(name = "Jack", age = 1)
